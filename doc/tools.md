...
#### LATEST-NAME-IN
( `wid` -- `nt|0` )
If the word list identified by `wid` is empty, return zero (`0`); otherwise, return `nt`, the identifier of the definition whose name was placed most recently into the word list.

- - -
#### LATEST-NAME
( -- `nt` )
If the compilation word list is empty, throw a `-80` (the compilation wordlist is empty) exception. Otherwise, return `nt`, the identifier of the definition whose name was placed most recently into the compilation word list.
